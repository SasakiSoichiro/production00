...
// インスタンス生成
//*******************
CRailBlock* CRailBlock::Create(D3DXVECTOR3 pos)
{
	CRailBlock* pBlock;
	...
	{// オブジェクトが最大数に達していない場合
		...
		pBlock = new CRailBlock;	// インスタンス生成
		pBlock->Init(pos);		// 初期化
		return pBlock;			// 情報を渡す
	}
...
// 初期化処理
//*****************
HRESULT CRailBlock::Init(D3DXVECTOR3 pos)
{
	// CXModelのポインタに情報を入れる
	CXModel* pXModel = CManager::GetCXModel();
	// 使うモデルのインデックスを入れる
	m_nIdxXFile = pXModel->Register("data\\MODEL\\antenna.x");
...
// 位置の設定
//******************
void CRailBlock::SetPos(D3DXVECTOR3 pos)
{
...
// 向きの設定
//******************
void CRailBlock::SetRot(D3DXVECTOR3 rot)
{
...
// 位置情報の取得
//******************
D3DXVECTOR3 CRailBlock::GetPos(void)
{
	return CObjectX::GetPos();// 位置情報取得
}
D3DXVECTOR3 CRailBlock::GetRot(void)
{
	return CObjectX::GetRot();// 位置情報取得
}

bool CRailBlock::CollisionCursor(void)
{
	// デバイスの取得
