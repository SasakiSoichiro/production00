...
// 静的メンバ変数初期化
//**********************
int CRail::m_nCntAll = 0;
int CRail::m_nUse = 0;
int CRail::m_nOldUse = 0;
int CRail::m_nPlayerCurrentIdx = 0;
float  CRail::m_fPlayerCurrentT = 0.0f;
...
// インスタンス生成
//*******************
CRail* CRail::Create(D3DXVECTOR3 point0, D3DXVECTOR3 point1, D3DXVECTOR3 point2, D3DXVECTOR3 point3,int nType)
{
	CRail* pRail;
	pRail = new CRail;
...
void CRail::Update(void)
{
	...
	case CPlayer::STATE_NONE:
		...
		if (t==true&&pInputKeyboard->Trigger(DIK_SPACE)==true|| t == true && pInputJoypad->GetTrigger(CInputJoypad::JOYKEY_Y))
		{
			...
			CRailState* pRailState = new CRailState;
		...
		case CPlayer::STATE_RAIL:
			...
			if (m_nUse == m_nIdx)
			{
				...
				// 散るパーティクル
				CSparkParticle::Create(pos,				// 位置
					20,									// 半径
					D3DXVECTOR2(1.0f, 1.0f),			// サイズ
					1,									// 量
					20,									// 寿命
					1,									// 時間
				...
				case TYPE_CHARGE:
					if (fLife < 80.2f)
					{
						...
						CChargeEffect::Create(D3DXVECTOR3(pos.x, pos.y, pos.z), fAngle + ANGLE_OFFSET, D3DXVECTOR2(15.0f, 15.0f), 30, D3DXCOLOR(0.4f, 0.4f, 1.0f, 1.0f));
						CChargeEffect::Create(D3DXVECTOR3(pos.x, pos.y, pos.z), fAngle - ANGLE_OFFSET, D3DXVECTOR2(15.0f, 15.0f), 30, D3DXCOLOR(0.4f, 0.4f, 1.0f, 1.0f));
				...
				case TYPE_DISCHARGE:
					if (fLife > 21.0f)
					{
						...
						m_nEffectCnt++;
						if (m_nEffectCnt > effectCool)
						{// 放電エフェクト
							CMotion* pMotion = pPlayer->GetMotion();
							if (pMotion->GetType()==CPlayer::MOTION_RAILL)
							{
								CRailEffect::Create(pos, D3DXVECTOR3(0.0f, fAngle + ANGLE_OFFSET - 0.4f, 0.0f), RAILEFFECT_SIZE);
								CRailEffect::Create(pos, D3DXVECTOR3(0.0f, fAngle - ANGLE_OFFSET - 0.4f, 0.0f), RAILEFFECT_SIZE);
							}
							else if (pMotion->GetType() == CPlayer::MOTION_RAILR)
							{
								CRailEffect::Create(pos, D3DXVECTOR3(0.0f, fAngle + ANGLE_OFFSET + 0.4f, 0.0f), RAILEFFECT_SIZE);
								CRailEffect::Create(pos, D3DXVECTOR3(0.0f, fAngle - ANGLE_OFFSET + 0.4f, 0.0f), RAILEFFECT_SIZE);
							}
				...
				{
					pPlayer->SetState(CPlayer::STATE_RAILCHANGE);
					CRailChangeState* pRailChangeState = new CRailChangeState;
...
// メッシュの設定
//******************
void CRail::SetMesh(D3DXVECTOR3 pos0, D3DXVECTOR3 pos1)
{
	D3DXVECTOR3 pos[2];
	D3DXVECTOR3 dir;
	D3DXVECTOR3 width;
...
float CRail::GetAngle(D3DXVECTOR3 pos0, D3DXVECTOR3 pos1)
{
