...
void CPlayer::Update(void)
{
	...
	CBlockManager* pBlockManager = CGame::GetCBlockManager();
	m_oldPos = m_pos;
...
// モーションの読み込み
//************************
void CPlayer::LoadMotion(const char* pMotionScript)
{
	FILE* pFile = NULL;
...
// 進む方向
//***********
void CPlayer::AngleVelocity(float speed, float omega)
{
	float fSpeed=speed;
	if (m_nBoostTime > 0)
	{
...
void CRandState::Init(void)
{
	m_pPlayer->SetATK(1);
}
void CRandState::Update(void)
{
	// キーボードの取得
	CInputKeyboard* pInputKeyboard;
	CInputJoypad* pInputJoypad;
	...
	if (m_pPlayer->GetState() != CPlayer::STATE_RAIL)
	{
		pos.y = pMesh->GetHeight(pos);
	}
	// プレイヤーの上ベクトル
	...
	if (m_pPlayer->GetLife() <= 0.0f)
	{// 体力が0以下になったら
		CDieState* pDieState = new CDieState;
		m_pPlayer->ChangeState(pDieState);
		CGame::SetState(CGame::STATE_END);
	}
}
void CRandState::Uninit(void)
{

}
void CRandState::Controll(void)
{
	CCamera* pCamera = CManager::GetCCamera();
	CInputJoypad* pInputJoypad;
	CInputKeyboard* pInputKeyboard;
	...
	{// 左側が後ろ方向に進むとき
		...
		{// 右側が後ろ方向のとき
			RightSpeed = fSpeed;
			pMotion->SetMotion(CPlayer::MOTION_RB);
			// プレイヤーカメラ追従
		...
		{// 右側が前方向のとき
			...
			// プレイヤーカメラ追従
			pCamera->Target(posC, rotC);
			pMotion->SetMotion(CPlayer::MOTION_LB);
		}
		else
		{
			...
			pMotion->SetMotion(CPlayer::MOTION_BACK);
		}
	...
	{// 左側が前方向に進むとき
		...
		{// 右側が後ろ方向のとき
			...
			// プレイヤーカメラ追従
			pCamera->Target(posC, rotC);
			pMotion->SetMotion(CPlayer::MOTION_RF);
		...
		{// 右側が前方向のとき
			...
			// プレイヤーカメラ追従
			pCamera->Target(posC, rotC);
			pMotion->SetMotion(CPlayer::MOTION_LF);
		}
		else
		{
			RightSpeed = -fSpeed;
			...
			pMotion->SetMotion(CPlayer::MOTION_FLONT);
		}
	...
	{// 右側が後ろ方向のとき
		...
		pMotion->SetMotion(CPlayer::MOTION_FB);
	...
	{// 右側が前方向のとき
		...
		pMotion->SetMotion(CPlayer::MOTION_BF);
	...
	else
	{
		// プレイヤーカメラ追従
		pCamera->Target(posC, rotC);
		pMotion->SetMotion(CPlayer::MOTION_NUETORAL);
		m_pPlayer->SpeedDown();
	}
...
void CJumpState::Init(void)
{
	...
	CMotion* pMotion = m_pPlayer->GetMotion();
	pMotion->SetMotion(CPlayer::MOTION_JUMP);

}
void CJumpState::Update(void)
{
	// キーボードの取得
	...
	if (move.y > 0.0f)
	{
		...
		if (bElec)
		{
			// モーションの取得
			CMotion* pMotion = m_pPlayer->GetMotion();
			pMotion->SetMotion(CPlayer::MOTION_ELECJUMP);
	...
	{
		m_pPlayer->SetState(CPlayer::STATE_LANDING);
		m_pPlayer->SetAngle(fAngle);
		CLandingState* pRandState = new CLandingState;
...
void CJumpState::Uninit(void)
{

}

void CJumpState::Rotation(void)
{
...
void CRailState::Init(void)
{
	// モーションの取得
	CMotion* pMotion = m_pPlayer->GetMotion();
	pMotion->SetMotion(CPlayer::MOTION_RAILL);
	
}
void CRailState::Update(void)
{
	// 入力の取得
	...
	{// 右向く
		pMotion->SetMotion(CPlayer::MOTION_RAILR);
	}
...
void CRailState::Uninit(void)
{

}


CRailChangeState::CRailChangeState()
{
...
void CRailChangeState::Init(void)
{
	// 入力の取得
	CInputKeyboard* pKeyboard = CManager::GetCInputKeyboard();
	CInputJoypad* pJoypad = CManager::GetCInputJoypad();
	...
	// モーションの取得
	CMotion* pMotion = m_pPlayer->GetMotion();
	pMotion->SetMotion(CPlayer::MOTION_RAILJUMP);
...
void CRailChangeState::Update(void)
{
	// プレイヤー情報取得
...
void CRailChangeState::Uninit(void)
{
...
// 更新処理
//*************
void CLandingState::Update(void)
{
	// カメラ情報取得
	CCamera* pCamera = CManager::GetCCamera();
	// プレイヤー情報取得
...
// 終了処理
//*************
void CLandingState::Uninit(void)
{
	m_pPlayer->SetElec(false);
}


CInoperableState::CInoperableState()
{
	m_nCnt = 0;
}
CInoperableState::~CInoperableState()
...
void CInoperableState::Init(void)
{
	m_nCnt = 0;
}
void CInoperableState::Update(void)
{
	// メッシュ取得
	CMesh* pMesh = CGame::GetCMesh();
	// プレイヤー情報取得
...
void CInoperableState::Uninit(void)
{
...
void CDieState::Init(void)
{
	// サウンドの取得
	CSound* pSound = CManager::GetCSound();
	pSound->Play(CSound::LABEL_SE04);
}
void CDieState::Update(void)
{
	// モーションの更新
	CMotion* pMotion = m_pPlayer->GetMotion();
	pMotion->SetMotion(CPlayer::MOTION_DIE);
}
void CDieState::Uninit(void)
{
