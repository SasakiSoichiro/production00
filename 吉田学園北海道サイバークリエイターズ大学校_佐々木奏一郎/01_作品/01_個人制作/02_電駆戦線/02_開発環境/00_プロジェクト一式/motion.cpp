...
void CMotion::Update(void)
{
	...
	for (int nCntModel = 0; nCntModel < m_nNumModel; nCntModel++)
	{
		...
		//相対値
		float fRateMotion = m_nCnt / ((float)m_aInfo[m_type].aKeyInfo[m_nKey].nFlame*nSlowMulti);

		//ブレンドするフレーム
		m_nFrameBlend = BLEND_FRAME* nSlowMulti;
		...
		{//bBlendがtrueだったら
			...
			// 相対値
			float fRateMotionBlend = m_nCntMotionBlend / ((float)m_aInfo[m_typeBlend].aKeyInfo[m_nKeyBlend].nFlame * nSlowMulti);

			// ブレンドの相対値
			float fRateBlend = m_nCntBlend / ((float)m_nFrameBlend * nSlowMulti);
	...
	{//ブレンドあり
		...
		if (m_nCntMotionBlend >= (m_aInfo[m_typeBlend].aKeyInfo[m_nKeyBlend].nFlame * nSlowMulti))
		{//フレーム数を超えたら
		...
		if (m_nCntBlend >= m_nFrameBlend * nSlowMulti)
		{//ブレンドのフレーム数に達したら
	...
	{//ブレンドなし
		...
		if (m_nCnt >= (m_aInfo[m_type].aKeyInfo[m_nKey].nFlame * nSlowMulti))
		{//フレーム数を超えたら
