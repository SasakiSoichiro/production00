...
//
//***************************************************
#ifndef _TUTORIALPLAYER_H_ // このマクロ定義がされていなかったら
#define _TUTORIALPLAYER_H_ // 二重インクルード防止のマクロ定義
...
public:
	...
	static CTutorialPlayer* Create(D3DXVECTOR3 pos);// インスタンス生成
	...
	void ChangeState(CTutorialPlayerStateBase* pState);			// 状態を変える
	...
	bool GetJumpCancel(void) { return m_bJumpCancel; }	// ジャンプキャンセル取得
...
{
public:
	CTutorialPlayerStateBase();				// コンストラクタ
	~CTutorialPlayerStateBase();			// デストラクタ
	void SetOwner(CTutorialPlayer* pPlayer);// プレイヤーのポインタ設定
protected:
	CTutorialPlayer* m_pPlayer;				// プレイヤーへのポインタ
};
...
{
public:
	CTutorialRandState();		// コンストラクタ
	~CTutorialRandState();		// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
	void Controll(void);// 操作
private:
	static constexpr float backSpeed = 0.5f;// 加速度
};
...
{
public:
	CTutorialJumpState();		// コンストラクタ
	~CTutorialJumpState();		// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
...
{
public:
	CTutorialRailState();		// コンストラクタ
	~CTutorialRailState();		// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
private:
...
{
public:
	CTutorialRailChangeState();		// コンストラクタ
	~CTutorialRailChangeState();	// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
private:
...
{
public:
	CTutorialLandingState();	// コンストラクタ
	~CTutorialLandingState();	// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
private:
...
{
public:
	CTutorialInoperableState();		// コンストラクタ
	~CTutorialInoperableState();	// デストラクタ
	void Init(void);	// 初期化
	void Update(void);	// 更新
	void Uninit(void);	// 終了
private:
